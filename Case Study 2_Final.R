...
# Summarize fields i think may be important
summary(df$Age) # 18 - 60, Avg age of 35
summary(df$Education) # Levels 1-5, Avg Education is 3
summary(df$StandardHours) # seems everyone is supposed to work 80 hours
summary(df$HourlyRate) # Avg hourly rate is $60... Pretty high

# Creating age groups
df$AgeGroup <- cut(df$Age, breaks=c(18, 25, 35, 45, 55, 65), 
                   labels=c("18-24", "25-34", "35-44", "45-54", "55-64"), include.lowest = TRUE)
table(df$AgeGroup)

# Scaling DailyRate
df$ScaledDailyRate <- scale(df$DailyRate, center = TRUE, scale = TRUE)
summary(df$ScaledDailyRate)


# Creating interaction feature
df$Edu_JobInteraction <- interaction(df$Education, df$JobRole)
head(df$Edu_JobInteraction)
...
# investigating total amount/ratio of attritions
df |> 
  count(Attrition) |> 
  mutate(Percentage = n / sum(n) * 100) |> 
  ggplot(aes(x = Attrition, y = Percentage, fill = Attrition))+
  geom_bar(aes(color = Attrition), 
           stat = "identity")+
...
#highest is Research and development
...
# Jobs that acount for the highest percentage of the attiritions are:
...
cor_matrix <- cor(df[numeric_vars])  # use complete.obs to handle missing data
print(cor_matrix)

# TotalWorkingYears, JobLevel, and Overtime appear to be the variables with the highest correlation 
  # to Attrition.

# For my Attrition analysis i have chosen to use:
  # "Age","TotalWorkingYears", "StockOptionLevel","YearsWithCurrManager"
  # "JobLevel","YearsInCurrentRole", "YearsAtCompany", "OTNum", "JobSatisfaction", "MonthlyIncome"
...
CM
##############################################################################################
# Select three variables and Attrition.
...
# Assuming 'test_data' is your data frame where you want to add the 'Attrition' predictions
test_data$Attrition <- knn_predictions$Decision
test_data <- arrange(test_data, ID) 
# Output the results or save the file
...
# NumCompaniesWorked, Attrition, and TotalWorkingYears appear to be the variables with the highest correlation 
# to MonthlyIncome.
...
# Remove columns with single unique levels
train_data <- train_data %>%
  dplyr::select(-one_of(names(factor_check[factor_check])))  # Exclude invalid columns

# Step 2: Check for and handle missing data
# You can either remove rows with missing values or impute them
# Here, we'll remove rows with missing data in critical columns
...
# Re-create the list of predictors, excluding columns like 'ID' and single-level factors
...
# Step 5: Validate the model and compute RMSE
...
# Step 6: Apply to the test dataset if RMSE is acceptable
if (rmse < 3000) {
